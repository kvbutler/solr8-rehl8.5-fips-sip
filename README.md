# SOLR 8.11.1 Installation on REHL 8.5 w/ SunPKCS11-NSS-FIPS Keystore Provider

## Setup

1. [Obtain SOLR binary release](https://dlcdn.apache.org/lucene/solr/8.11.1/solr-8.11.1.tgz)
`wget https://dlcdn.apache.org/lucene/solr/8.11.1/solr-8.11.1.tgz`
2. Move tar to the /app directory
`mv solr-8.11.1.tgz /app`
`cd /app/`
3. Decompress tar file
`tar zxf solr-8.11.1.tgz`
4. Copy the service installation script
`cp solr-8.11.1/bin/install_solr_service.sh .`
5. Delete unpacked files (service script will work from the tar)
`rm -rf solr-8.11.1`
6. Install nss-tools
...
`./install_solr_service.sh solr-8.11.1.tgz -i /app`
2. Copy [ssl-context-factory-extensions.jar](files/ssl-context-factory-extensions.jar) to `/app/solr-8.11.1/server/lib/ext`
	- [For more information see open issue](https://github.com/eclipse/jetty.project/issues/2010)
3. Replace the file `/app/solr-8.11.1/server/etc/jetty-ssl.xml` with the following contents.
```xml
<?xml version="1.0"?>
...
<!-- ============================================================= -->
<Configure id="sslContextFactory" class="com.icf.jetty.ssl.FIPSSslContextFactory">
...
```
4. Replace the file `/etc/default/solr.in.sh` with the following
```bash
...
#SOLR_SSL_KEY_STORE=/app/solr/server/solr-ssl.keystore.jks
#SOLR_SSL_KEY_STORE_PASSWORD=secret
#SOLR_SSL_TRUST_STORE=/app/solr/server/solr-ssl.keystore.jks
...
SOLR_PID_DIR="/app/solr"
SOLR_HOME="/app/solr/data"
LOG4J_PROPS="/app/solr/log4j2.xml"
SOLR_LOGS_DIR="/var/log/solr"
SOLR_PORT="8983"
...
# Uncomment to set SSL-related system properties
# Be sure to update the paths to the correct keystore for your environment
SOLR_SSL_KEY_STORE=NONE
#SOLR_SSL_KEY_STORE_PASSWORD=secret
SOLR_SSL_TRUST_STORE=NONE
#SOLR_SSL_TRUST_STORE_PASSWORD=secret
# Require clients to authenticate
SOLR_SSL_NEED_CLIENT_AUTH=false
# Enable clients to authenticate (but not require)
...
SOLR_SSL_KEY_STORE_TYPE=PKCS11
SOLR_SSL_TRUST_STORE_TYPE=PKCS11
```
...
[Service]
User=solr
PIDFile=/app/solr/solr-8983.pid
